...
public class BatteryCalibrationFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup parent,
        ...
        return (inflater.inflate(R.layout.battery_calibration, parent, false));
    }
    ...
    public void CalibrateProcess() {
        if (RootTools.isAccessGiven()) {
            @SuppressLint("SdCardPath") Command calibratecommand = new Command(0,
            ...
            try {
                RootTools.getShell(true).add(calibratecommand);
                if (ThemeUtility.getTheme(getActivity().getApplicationContext()) == 1) {
                    ...
                    handler.postDelayed(new Runnable() {
                        public void run() {
                            dialog.dismiss();
                            new SnackBar.Builder(getActivity()).withMessage(getContext().getResources().getString(R.string.calsucess)).withBackgroundColorId(R.color.resultgood).show();
                            if (ThemeUtility.getTheme(getActivity().getApplicationContext()) == 1) {
                                new android.app.AlertDialog.Builder(getView().getContext())
